/**
 * @domflax/frontend-vue — IR → `.vue` backend (SURGICAL, verbatim-preserving).
 *
 * The pass manager mutates the IR tree that was lowered from an SFC's `<template>` block. The backend
 * emits the SAME file back with ONLY the diffs the passes produced, applied as span edits over the
 * ORIGINAL source via `magic-string`. It NEVER re-serializes an AST. Because the parse pass only ever
 * records spans INSIDE the template block, every byte outside it — `<script>` / `<style>` blocks,
 * block tags, whitespace — is untouchable by construction, and every untouched template byte
 * (comments, attribute order, quoting) stays byte-for-byte identical.
 *
 ...
 *
 * (Logic mirrors `@domflax/frontend-html`'s backend — kept package-local so the two frontends stay
...
/** All ids the backref table knows about (every originally-parsed element with derivable tags). */
...
function editClasses(ms: MagicString, doc: IRDocument, sf: SourceFile, el: IRElement): boolean {
  ...
  if (valueSpan && valueSpan.file === sf.id) {
    if (sameTokens(currentTokens(sf, valueSpan), tokens)) return false; // unchanged → verbatim
...
/** Surgical whole-file codegen. Returns null when the document has no retained source. */
...
/** Emit the (possibly edited) file. Falls back to the empty string only for a source-less doc. */
