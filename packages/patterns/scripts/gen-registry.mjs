 ...
 * drop a `<name>.pattern.ts` under any DOMAIN folder in `src/library/` (e.g. `wrapper/`, `flex/`,
 * `grid/`, `fragment/`, `layout/`) and re-run `npm run generate`. The pass PHASE is still derived
 * from each pattern's `category` first segment (`flatten`/`compress`), independent of its folder.
 *
